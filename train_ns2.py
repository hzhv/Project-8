...
def train_model(model, data_loader, table_criterion, idx_criterion, optimizer, num_epochs, writer, device, num_neg_samples):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            pos_table_output = table_output_embeds[:, :-1, :]  # (batch_size, seq_len, embed_dim)
            pos_table_target = model.table_id_embed(gt_table[:, 1:])  # (batch_size, seq_len, embed_dim)
            ...
            neg_table_output = table_output_embeds[:, :-1, :].unsqueeze(2).expand(-1, -1, num_neg_samples, -1)  # (batch_size, seq_len, num_neg_samples, embed_dim)
            neg_table_targets = model.table_id_embed(neg_table_samples).unsqueeze(1).expand(-1, model.output_length - 1, -1, -1)  # (batch_size, seq_len, num_neg_samples, embed_dim)

            # Positive samples for idx_id
            pos_idx_output = idx_output_embeds[:, :-1, :]  # (batch_size, seq_len, embed_dim)
            pos_idx_target = model.idx_id_embed(gt_idx[:, 1:])  # (batch_size, seq_len, embed_dim)
            ...
            neg_idx_output = idx_output_embeds[:, :-1, :].unsqueeze(2).expand(-1, -1, num_neg_samples, -1)  # (batch_size, seq_len, num_neg_samples, embed_dim)
            neg_idx_targets = model.idx_id_embed(neg_idx_samples).unsqueeze(1).expand(-1, model.output_length - 1, -1, -1)  # (batch_size, seq_len, num_neg_samples, embed_dim)
...
def validate_model(model, data_loader, table_criterion, idx_criterion, device, fileName, reverse_table_id_map, idx_tokenizer, num_neg_samples):
    model.eval()
    model.to(device)
    val_loss = 0.0
    total_accuracy = 0
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            predicted_idx_original = torch.tensor([idx_tokenizer.decode([v.item()]) for v in predicted_idx]).view(predicted_idx.size())

            gt_table_original = torch.tensor([reverse_table_id_map[v.item()] for v in gt_table.view(-1)]).view(gt_table.size())
            gt_idx_original = torch.tensor([idx_tokenizer.decode([v.item()]) for v in gt_idx.view(-1)]).view(gt_idx.size())
...
def main(config):
    ...
    train_loader, output_length, table_unq, idx_unq, table_id_map, reverse_table_id_map, idx_tokenizer = load_dataset(
        train_set, 
        input_window_size, 
        prediction_ratio, 
        bs
